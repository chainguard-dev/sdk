...
func NewClients(ctx context.Context, apiURL string, token string) (Clients, error) {
	apiURI, err := url.Parse(apiURL)
	if err != nil {
		return nil, fmt.Errorf("failed to parse packages service address, must be a url: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("NewClients: failed to connect to the packages server: %w", err)
	}
