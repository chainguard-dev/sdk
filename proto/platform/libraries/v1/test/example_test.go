...
func ExampleMockLibrariesClients() {
	...
	if err != nil {
		clog.FatalContextf(ctx, "failed to list entitlements: %v", err)
	}
...
func ExampleMockArtifactsClient() {
	...
	if err != nil {
		clog.FatalContextf(ctx, "failed to list artifacts: %v", err)
	}

	// List versions for an artifact
	versions, err := mock.ListVersions(ctx, &v1.ArtifactVersionFilter{Id: "artifact-123"})
	if err != nil {
		clog.FatalContextf(ctx, "failed to list versions: %v", err)
	}
...
func ExampleMockEntitlementsClient() {
	...
	if err != nil {
		clog.FatalContextf(ctx, "failed to create entitlement: %v", err)
	}

	// Delete an entitlement
	_, err = mock.Delete(ctx, &v1.DeleteEntitlementRequest{Id: "ent-789"})
	if err != nil {
		clog.FatalContextf(ctx, "failed to delete entitlement: %v", err)
	}
...
func ExampleMockNpmPackagesClient() {
	...
	if err != nil {
		clog.FatalContextf(ctx, "failed to list packages: %v", err)
	}
