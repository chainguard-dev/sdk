...
import (
	...
	iamv2b "chainguard.dev/sdk/proto/chainguard/platform/iam/v2"
...
func TestIsTermsNotAccepted_V1AndV2Interop(t *testing.T) {
	...
	if !ok1 || !ok2 {
		t.Fatalf("both should return true: v1=%v, v2=%v", ok1, ok2)
	}
	if len(docs1) != 1 || len(docs2) != 1 {
		t.Fatalf("both should return 1 doc: v1=%d, v2=%d", len(docs1), len(docs2))
	}
	if docs1[0] != docs2[0] {
		t.Errorf("extracted docs should match:\nv1:      %+v\nv2: %+v", docs1[0], docs2[0])
	}
}
